...
- `{archive_dir}` — completed feature/bug artifacts; for features, a single summary per feature at `{archive_dir}/{feature_name}/{feature_name}.md`; for bugs, a single summary per fix at `{archive_dir}/fix-{bug_name}/fix-{bug_name}.md` (default: `_archive`)
- `{reports_dir}` — reports and long-form outputs, one folder per report (default: `reports`)
...
The `docs/` folder is only for this repo's Cursor workflow documentation and AI context; reports go in `{reports_dir}/{report_name}/`. See CONVENTIONS.md and `docs/README.md`.
...
- **agents/** — Personas for research, planning, implementation, debugging, task decomposition, archiving (feature and bug summary), and AI context writing
