...
**Phase 3: Completion**
- [ ] Step 3.1: Updated root `CHANGELOG.md` under `## [Unreleased]` in `### Fixed` (link to bug summary; pipeline produces summary in Phase 7)
- [ ] Step 3.2: Updated relevant documentation in `{context_docs_dir}/` and READMEs (if applicable)
- [ ] Step 3.3: Ran full test suite again; all tests passed
...
Task and plan files remain in `{bugs_dir}/{bug_name}/`; the bug pipeline produces a single archive summary in Phase 7 and then deletes the bug folder.

### Step 3.3: Final Validation

Run full test suite again. All tests must pass.

### Step 3.4: Verify No Code Quality Errors

Verify no code quality errors remain.
...
- Task and plan files remain in `{bugs_dir}/{bug_name}/` until Phase 7 of the bug pipeline; the pipeline then writes one summary to `{archive_dir}/fix-{bug_name}/fix-{bug_name}.md` and deletes the bug folder
