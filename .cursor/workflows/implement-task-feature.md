...
**Phase 3: Completion**
- [ ] Step 3.1: Updated root `CHANGELOG.md` under `## [Unreleased]` with description and feature summary link
- [ ] Step 3.2: Updated relevant documentation in `{context_docs_dir}/` and READMEs (if applicable)
- [ ] Step 3.3: Ran all project tests (Python + Node) again; all tests passed
- [ ] Step 3.4: Verified no code quality errors remain
- [ ] Step 3.5: Updated `{scratchpad}` with "Task {task_file} completed, tests passing"
- [ ] Step 3.6: Committed all changes with Conventional Commits message
...
- Description of the change
  - Feature summary: [feature_name]({archive_dir}/{feature_name}/{feature_name}.md)
```

The feature summary document is created in Phase 6 of the feature pipeline; the link will be valid after the pipeline completes.
...
- Task and plan files remain in `{features_dir}/{feature_name}/` until Phase 6 of the feature pipeline (the archivist produces a single summary and the pipeline deletes the feature folder)
