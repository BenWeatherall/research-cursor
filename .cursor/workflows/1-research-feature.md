...
**Do not use `reports/` as input.** Do not read or use content from `{reports_dir}` (default: `reports/`) unless the user explicitly provides a path or content from that directory. Research must stand on its own and not be biased by prior reports in the repository.
