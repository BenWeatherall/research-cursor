...
- [ ] Step 4.3: Produced feature summary and archived it; deleted feature request and folder (see Step 4.3)
...
4. Add a link to the feature summary (created in Step 4.3):

```markdown
### Added/Changed/Fixed
- Description of the change
  - Feature summary: [feature]({archive_dir}/{feature}/{feature}.md)
```
...
### Step 4.3: Archive Feature Summary and Delete Artifacts

Produce a single summary document for the archive and remove the feature folder. Align with `@.cursor/workflows/6-archive-feature-summary.md`:

1. **Gather**: Read the feature request, planning docs, master plan, task list, task plans, and task documents from `{features_dir}/{feature}/` and `{features_dir}/{feature}.md`.
2. **Summarise**: Write one markdown document with three sections: **The request**, **Planned approach**, **What was implemented**. Keep it concise.
3. **Write**: Ensure `{archive_dir}/{feature}/` exists; write the document to `{archive_dir}/{feature}/{feature}.md`.
4. **Delete**: Delete `{features_dir}/{feature}.md` and the entire tree `{features_dir}/{feature}/`. The archive contains only the summary file.
...
**Changelog Entry:** Add under `## [Unreleased]` with feature summary link to `{archive_dir}/{feature}/{feature}.md`.
...
- **Archive**: A single feature summary document is written to `{archive_dir}/{feature}/{feature}.md`; the feature request and folder are then deleted. For the multi-task pipeline, Phase 6 invokes the `/archivist` sub-agent to produce this summary.
- **Links in changelog**: Link to the feature summary at `{archive_dir}/{feature}/{feature}.md`
