...
**Do not use `reports/` as input.** Do not read or use content from `{reports_dir}` (default: `reports/`) unless the user explicitly provides a path or content from that directory. Planning must be based on the current feature and project context only.
