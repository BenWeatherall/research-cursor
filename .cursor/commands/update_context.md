...
**Data source restriction:** Do not use anything under `{reports_dir}` (default: `reports/`) as input, context, or reference when generating or updating AI context documents, unless the **user explicitly provides** a path or content from that directory. When invoking `/ai-context-writer`, instruct it to observe the same rule: do not read or cite from `reports/`. Context documentation must reflect the current project and not be biased by prior research or reports in the repository.
