...
- **Do not use `reports/` as input.** No phase (research, plan, task list, implementation, archive) may read or use content from `{reports_dir}` (default: `reports/`) as context, data source, or reference unless the **user explicitly provides** a path or content from that directory. Each piece of research and each feature run must stand on its own and not be biased by prior reports in the repository.
- `reports/` remains a **valid output destination**: when the deliverable is a report, the implementer writes to `{reports_dir}/{feature_name}/` as stated in Notes.
...
2. Invoke the `/implementer` subagent with a **fresh agent context**. Pass the feature name and single task file path. Ask it to:
   ...
   - Run the **Commit Procedure** (defined in `implement-task-feature.md` Step 3.5: Commit Changes) to commit all changes for the completed task
...
- **Reports**: The `docs/` folder is only for Cursor workflow documentation and AI context. When the feature deliverable is a report (e.g. documentation-only), the implementer writes the report to `{reports_dir}/{feature_name}/` (default: `reports/{feature_name}/`). Reports may exceed standard doc length and include multiple files.
- **Optional reinforcement**: Adding `reports/` to `.cursorignore` prevents Cursor from indexing that folder, so report content is not included in codebase context by default. Writing to `reports/` still works; only use of reports as *input* is restricted by the rule above.
